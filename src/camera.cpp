...
float OrthoCamera::generateRay(const CameraSample &sample, Ray *ray) {
	...
	glm::mat4 scale1 = glm::scale(1.f/m_film->xResolution, 1.f/m_film->yResolution, 1.f);
	vec4 normalized =  scale1 * vec4(Pras, 1.0);
	glm::mat4 scale2 = glm::scale( (2.f -1.f/normalized.x) *aspect /*(float)tan(angle/2.f * M_PI /180) */ , (1.f/normalized.y - 2.f) /** (float)tan(angle/2.f * M_PI / 180)*/, 1.f);
